...
export default async function ArchivePage({ 
  ...
  const selectedYear = searchP.year ? Number(searchP.year) : undefined;

  const t = await getTranslations('Archive');

  // getIssues руу locale бас дамжуулна
  const { issues, pagination } = await getIssues(currentPage, 19, selectedYear, locale);
    ...
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      {years.length > 0 ? (
          ...
          <div key={year} className="mb-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
              {groupedIssues[year].map((issue: Issue) => (
                <Link key={issue.id} href={`/${locale}/archive/${issue.documentId}`} className="group">
        ...
        <div className="py-20 text-center">
          <p className="text-gray-500">{t('noIssuesYear')}</p>
          <Link href={`/${locale}/archive`} className="text-[#003d71] underline mt-4 inline-block">
            {t('viewAll')}
          </Link>
        </div>
      )}
      ...
      {pagination && pagination.pageCount > 1 && (
        <div className="mt-16 flex justify-center items-center gap-4">
          {currentPage > 1 && (
            <Link 
              href={`/${locale}/archive?page=${currentPage - 1}${selectedYear ? `&year=${selectedYear}` : ''}`}
          ...
          <span className="text-sm font-medium text-gray-500">
            {t('pageLabel')} {currentPage} / {pagination.pageCount}
          </span>
